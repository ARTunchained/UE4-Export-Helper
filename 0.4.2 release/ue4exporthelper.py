...
bpy.types.Scene.centerLODToOb = BoolProperty(default=True, description='Centre LODs to the main object\'s coords on export, so you can keep them side-by-side while working in Blender')
bpy.types.Scene.reverseLODs = BoolProperty(default=True, description='By default UE4 imports the first LOD (00) as the farsest object. Leave this on for a human 1:1 translation of your LOD order from Blender to UE4, so the last LOD in Blender will be the farsest in UE4')
bpy.types.Scene.peOneStepToogle = BoolProperty(default=False)
bpy.types.Scene.returnToLocal = StringProperty(default="")
bpy.types.Scene.triangulate = BoolProperty(default=True, description='Convert Quads to Tris on preparation')
bpy.types.Scene.includeConnected = BoolProperty(default=False, description='Include all objects that are connected to the current selection via parent / child relationship')
bpy.types.Scene.rotateMinusNinety = BoolProperty(default=False, description='Apply a +90° Rotation around Z on preparation to humanly translate the forward axis')
...
class UE4Export_addPreset(AddPresetBase, bpy.types.Operator):
    ...
    preset_values = [
        ...
        "scene.activateLOD",
        "scene.centerLODToOb",
        "scene.includeConnected",
        "scene.rotateMinusNinety"
        ]
...
class UE4MessageOperator(bpy.types.Operator):
    ...
    def draw(self, context):   
        row = self.layout.split(.96)
        col = row.column()
        subrow = col.row()
        subrow.label("UE4 Export Helper Checkup")
        ...
        if bpy.context.scene.altLayout:
            box = self.layout.box()
        row = self.layout.row() 
        row.label("OK", icon='FILE_TICK')  
        row.label("WARNING", icon='ERROR')  
        row.label("ERROR", icon='CANCEL')  
        row = self.layout.row()   
        row.label("Move mouse out of this window to close it!") 
...
def includeConnected():
    #Save currently active object
    currentActiveOb = bpy.context.scene.objects.active
    
    #Save current selection of objects
    currentSelection = bpy.context.selected_objects
    
    #Select all objects that are connected to this object via parent / child relationship
    
    #Create empty list, add user selection
    newSelection = []
    newSelection.extend(currentSelection)
    
    #Cycle through all objects selected by the user
    for ob in currentSelection:
        #Check if object has a parent
        currentParent = None
        if ob.parent != None:
            currentParent = ob.parent
            #Follow the hierachy upwards
            while currentParent.parent != None:
                currentParent = currentParent.parent
            #Now we have the topmost parent of this object - make this parent active
            bpy.context.scene.objects.active = currentParent
            #and select all children (select_grouped unselects other objects)
            bpy.ops.object.select_grouped(type='CHILDREN_RECURSIVE')
            
        #If current object has no parent
        else:
        ...
        #Save the previous selection before moving on to the next object
        newSelection.extend(bpy.context.selected_objects)
        
        #Deselect everything, just to be safe
        bpy.ops.object.select_all(action='DESELECT')            
    
    #Select everything from the generated list
    for ob in newSelection:
        ob.select = True 
        
    #Reset the active object
    bpy.context.scene.objects.active = currentActiveOb
            ...
            for ob in objects:
                ...
                if bpy.context.scene.centerOb:
                    if bpy.context.scene.centerRel:
                        ...
                        lodPrepLocation = thisObLocation 
...
def LODPrepare(ob, orgLocation):
    ...
    if LODSocket != "":
        ...
        """ Reverse LOD order optional """
        names = LODSocket.split(',')

        if bpy.context.scene.reverseLODs:
        ...
        for name in names:
            ...
            """ Centre LODs or move them relatively """
            if bpy.context.scene.centerLODToOb:
                element.location = orgLocation
            else:
                element.location -= orgLocation
...
class Prepare_UE4_Export(bpy.types.Operator):
    ...
    def execute(self, context):                  
        if not len(bpy.context.selected_objects) > 0:
            ...
            exitLocalView()           
            if bpy.context.scene.includeConnected:
                includeConnected()
            clearSelection()
            
            scene = bpy.context.scene
            if bpy.context.scene.objects.active is None:    
                activeOb = bpy.context.scene.objects.active = bpy.context.selected_objects[0]   
            ...
            for ob in objects:                
                if ob.type == 'MESH' or ob.type == 'CURVE' or ob.type == 'FONT':                          
                    ...
                    if bpy.context.scene.rotateMinusNinety: #important to keep this after the original "Apply Transformation"
                        bpy.ops.object.transform_apply(location=False, rotation=True, scale=False)
                        duplicate.rotation_euler = (0, 0, radians(90))
                        bpy.ops.object.transform_apply(location=False, rotation=True, scale=False)
                    ...
                    if bpy.context.scene.triangulate:
            ...
            if lastActiveParent is not None:
                scene.objects.active = lastActiveParent
...
class UE4PreparePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        col.label('UE4 Export Helper Presets')
        row = col.row(align=True)
        row.menu("UE4Export_presets",
                 text=bpy.types.UE4Export_presets.bl_label)
        row.operator("object.ue4_add_preset", text="", icon='ZOOMIN')
        row.operator("object.ue4_add_preset", text="", icon='ZOOMOUT').remove_active = True
        ...
        col = layout.column_flow(align=True)
        col.label('UE4 Collision Meshes')
        row = col.row(align=True)
        ...
        if bpy.context.scene.objects.active is not None:   
            colMeshes = ""
            try:
                colMeshes = bpy.context.scene.objects.active["colSocket"] 
            except:
                pass
            if colMeshes != "":
                row = col.row(align=True)
                ...
                for name in colMeshes.split(','):
                    row = col.row(align=True)
                    row.label(text=name)         
                    row.operator("object.ue4_remove_collision", text="", icon='ZOOMOUT').activeName=name
        ...
        if context.scene.activateLOD:
            ...
            if bpy.context.scene.objects.active is not None:   
                lodMeshes = ""
                try:
                    lodMeshes = bpy.context.scene.objects.active["lodSocket"] 
                except:
                    pass
                if lodMeshes != "":
                    row = col.row(align=True)
            ...
            split = layout.split(align=True)
            col = split.column(align=True)
            col.prop(context.scene, "centerLODToOb", text="Centre LODs to object")
            col = split.column(align=True)
            col.prop(context.scene, "reverseLODs", text="Reverse LOD order")
        ...
        split = layout.split(align=True)
        col = split.column(align=True)
        col.prop(context.scene, "triangulate", text="Triangulate")
        col = split.column(align=True)
        col.prop(context.scene, "includeConnected", text="Include Connected Obs")
        ...
        split = layout.split(align=True)
        col = split.column(align=True)
        col.prop(context.scene, "rotateMinusNinety", text="Apply +90° Z-Rotation")
