...
def clearSelection():
    ...
    if ob is not None:        
        if "UCX_" in ob.name or "USP_" in ob.name or "UBX_"in ob.name :
            bpy.context.active_object = ob.parent
...
class Call_UE4_Export(bpy.types.Operator):
    ...
    def execute(self, context):   
        ...
        if bpy.context.scene.singleOb:
            ...
            """Find the originaly active object"""
            activeOb = None
            for ob in bpy.context.selected_objects:
                try:
                    if ob["active"]:
        ...
        if bpy.context.scene.deleteCopy:                    
            ...
            for ob in bpy.context.selected_objects:
                ...
                try:
                    ...
                    pass
                #print(ob.name, " is colcopy? ", isColCopy)
                if not (isUEEHCopy or isColCopy):
                    ob.select = False
        ...
        if bpy.context.scene.objects.active is not None:   
            ...
            if colMeshes != "":
                ...
                for name in colMeshes.split(','):
                    ...
                    row.label(text=name)         
                    #split.label(text="X")
                    #row.prop(bpy.context.scene.objects.active, '["colSocket"]', text="Col. Mesh")
