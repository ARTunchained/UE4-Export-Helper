...
class OT_UE4Export_addPreset(AddPresetBase, bpy.types.Operator):
    """Add a new preset."""
    bl_idname = 'object.ue4_add_preset'
    bl_label = 'Add UE4 Export Helper Preset'
    bl_options = {'REGISTER', 'UNDO'}
    preset_menu = '_MT_UE4Export_presets'
...
class _MT_UE4Export_presets(bpy.types.Menu):
    """Presets for UE4 Export Helper"""
    bl_label = 'UE4 Export Helper Presets'
    bl_idname = '_MT_UE4Export_presets'
    preset_subdir = 'ue4exporthelper_presets'
    preset_operator = 'script.execute_preset'

    draw = bpy.types.Menu.draw_preset
    
class OT_UE4_CheckupFix(bpy.types.Operator):
...
class OT_UE4_Checkup(bpy.types.Operator):
...
class OT_UE4MessageOperator(bpy.types.Operator):
...
class OT_XOperator(bpy.types.Operator):
...
class OT_AddCollisionMesh_UE4(bpy.types.Operator):
...
class OT_UE4Export_removeCollision(bpy.types.Operator):
...
class OT_AddLODMesh_UE4(bpy.types.Operator):
...
class OT_UE4Export_removeLOD(bpy.types.Operator):
...
class OT_PrepareAndExport_UE4(bpy.types.Operator):
...
class OT_Call_UE4_Export(bpy.types.Operator):
    """Call fbx exporter"""
...
class OT_Prepare_UE4_Export(bpy.types.Operator):
            ...
            for ob in objects:                
                if ob.type == 'MESH' or ob.type == 'CURVE' or ob.type == 'FONT':                          
                    ...
                    for vert in duplicate.data.vertices:
                        vert.select = True # vertex
                        #vert.select_set(True) # objects
...
class PT_UE4PreparePanel(bpy.types.Panel):
    """UE4 Export Helper"""
    ...
    def draw(self, context):
        ...
        row.menu("_MT_UE4Export_presets",
                 text=bpy.types._MT_UE4Export_presets.bl_label)
        ...
        if bpy.context.scene.collisionType in ("UCX", "UBX", "USP"):
            row = col.row(align=True)
            row.label(text='collision object (prop_search()')
            ## was: row.prop_search(context.scene, "collisionObject", context.scene, "objects", text="")
            #
            ## doc: prop_search( propContainer, propertyName, search_data, search_property, text="", text_ctxt="", translate=True, icon='NONE')
            ...
            if bpy.context.view_layer.objects.active is not None:   
                ...
                if lodMeshes != "":
                    ...
                    for name in lodMeshes.split(','):
                        row = col.row(align=True)
                        row.label(text=name)         
                        row.operator("object.ue4_remove_lod", text="", icon='ZOOM_OUT').activeName=name
...
classes = (
    OT_UE4Export_addPreset,
    _MT_UE4Export_presets,
    OT_UE4_CheckupFix,
    OT_UE4_Checkup,
    OT_UE4MessageOperator,
    OT_XOperator,
    OT_AddCollisionMesh_UE4,
    OT_UE4Export_removeCollision,
    OT_AddLODMesh_UE4,
	OT_UE4Export_removeLOD,
	OT_PrepareAndExport_UE4,
	OT_Call_UE4_Export,
	OT_Prepare_UE4_Export,
	PathSettings,  # property group - with path-string
	PT_UE4PreparePanel  # panel type
)        
